...
#Piotroski F Score takes all three years into account, while as magic formula only takes 1
...
for ticker in tickers:
    ...
    #getting balance sheet data from yahoo finance for the given ticker
        ...
        for t in tabl:
            ...
            for row in rows:
                #store all the text information
                temp_dir[row.get_text(separator='|').split("|")[0]]=row.get_text(separator='|').split("|")[1]
                #take the past 3 values, subscripting [1] [2] [3]
                #key is [0] heading in yf.com
...
stats = ["Net income available to common shareholders",
         ...
         "Total current liabilities", #Pension obligations, capital leases, etc.
         "Common stock",
         "Total revenue",
         "Gross profit"] # change as required

indx = ["NetIncome","TotAssets","CashFlowOps","LTDebt","OtherLTDebt",
        "CurrAssets","CurrLiab","CommStock","TotRevenue","GrossProfit"]#rename the stocks
...
def info_filter(df,stats,indx):
    """function to filter relevant financial information for each 
       ...
       #takes only the relevant info, and converting to numeric values
    tickers = df.columns
    all_stats = {}
    for ticker in tickers:
        try:
            temp = df[ticker] #only taking the ticker we are traversing
            ticker_stats = []
            for stat in stats:
                #For each header in headers
                ticker_stats.append(temp.loc[stat]) #append the relevant stats for that particular
                #ticker
                #temp.loc[stat] returns all the information given under the header
                #then we append it into the new header
    ...
    #remove all commas and replace with blank info
    for ticker in all_stats_df.columns:
        all_stats_df[ticker] = pd.to_numeric(all_stats_df[ticker].values,errors='coerce')
        #this simply converts all the string values into numeric values
        #"1000" converts to 1000
    return all_stats_df
...
def piotroski_f(df_cy,df_py,df_py2):
    ...
    for ticker in tickers:
        ...
        #if the value is true, and you convert to int, it is 1, if not it is 0
        #ROA = Return of Assets
        CFO_FS = int(df_cy.loc["CashFlowOps",ticker] > 0)
        #Cash Flow Operations, if greater than 0 , assign 1
        ...
        #df_cy is first year, df_py is second year, df_py2 is thirdyear
        ...
        #Looks at the cashflow operations and compares it with net income
        #Loc takes the info in the datafram
